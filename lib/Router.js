  ...
  data: function() {
    ...
    Speeches.find({meeting: this.params._meetingId, status: {$in: ["ongoing", "pending"]}}).forEach(function(doc) {
      ...
      if(seconds < 10) {
        ...
        console.log(Speeches.find({meeting: 'test', status: {$in: ["ongoing", "pending"]}}))
        speeches.push({
            user: Users.findOne({_id: doc.user}).name,
            timeLeft: minutesLeft + ":" + secondsLeft,
            timeString: doc.timeString,
            time: minutes + ":" + seconds,
			orderChoose: doc.orderChoose,
            subject: doc.subject,
